...
from .schemas import EmailRequest, PasswordResetConfirm, TokenPayload, TokenResponse, UserCreate, UserLogin, UserResponse
...
def login(payload: UserLogin, request: Request, db: Session = Depends(get_db)):
    ...
    access_token = create_access_token({"sub": str(user.id)}, expires_delta=timedelta(minutes=auth.ACCESS_TOKEN_EXPIRE_MINUTES))
...
def confirm_email(payload: TokenPayload, db: Session = Depends(get_db)):
...
@app.get("/health")
def health():
    return {"status": "ok", "rate_limit_remaining": {k: len(v) for k, v in rate_limiter.requests.items()}}
